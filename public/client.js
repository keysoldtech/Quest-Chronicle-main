...
const NotificationManager = {
    ...
    isAnyModalOpen() {
        ...
        const modalSelectors = [
            ...
            '#dice-roll-modal:not(.hidden)',
            '#level-up-modal:not(.hidden)',
            '#specialization-modal:not(.hidden)',
            '#event-modal:not(.hidden)',
            '#choose-path-modal:not(.hidden)',
            '#skill-challenge-modal:not(.hidden)',
            '#card-inspector-modal:not(.hidden)',
            '#player-inspector-modal:not(.hidden)',
            '#player-avatar-modal:not(.hidden)',
            '#class-selection-modal:not(.hidden)',
            '#stat-detail-modal:not(.hidden)',
            '#settings-modal:not(.hidden)',
            '#help-modal:not(.hidden)',
            '#game-paused-modal:not(.hidden)',
            '#confirmation-modal:not(.hidden)',
            '#onboarding-overlay:not(.hidden)',
            '#end-turn-prompt:not(.hidden)',
            '#choose-discard-modal:not(.hidden)',
            '#discovery-modal:not(.hidden)',
            '#target-selection-modal:not(.hidden)',
            '#game-over-modal:not(.hidden)',
            '#narrative-modal:not(.hidden)',
            '#loadout-modal:not(.hidden)',
            '#shop-modal:not(.hidden)',
            '#save-slot-modal:not(.hidden)',
            '#combat-grid-modal:not(.hidden)'
        ];
        
        for (const selector of modalSelectors) {
            if (document.querySelector(selector)) {
                console.log(`[ModalCheck] Found open modal: ${selector}`);
                return true;
            }
        }
        return false;
    ...
    isGated() {
        ...
        return this.isAnyModalOpen() || this.gatingState.diceOpen || this.gatingState.yourTurnVisible || this.gatingState.spectatorActive;
    },
...
function showYourTurnPopup() {
    ...
    const show = () => {
        ...
        // CRITICAL FIX: Mark turn popup as ready after a brief delay
        setTimeout(() => {
    ...
    // If gating active, defer via NotificationManager processing cycles
    if (NotificationManager.isGated()) {
        const wait = () => {
            if (NotificationManager.isGated()) return setTimeout(wait, 150);
            show();
        };
        wait();
    } else {
...
function showEndTurnPrompt() {
    ...
    // Gate via NotificationManager
    const attempt = () => {
        if (NotificationManager.isGated()) {
            clientState.endTurnPromptTimer = setTimeout(attempt, 200);
            return;
        }
        modal.classList.remove('hidden');
...
function showDiceRollModal(data) {
    ...
    const sides = parseInt(data.dice.substring(1));
    ...
    clientState.diceAnimationInterval = setInterval(() => {
        ...
        if (rollCount > 15) { // 1.5 seconds of animation
            ...
            if (OfflineActionHandler && OfflineActionHandler.isOffline()) {
                ...
                // The offline handler may open another modal (e.g., damage); safe to close this one now
                get('dice-roll-modal').classList.add('hidden');
            } else {
    ...
    if (choice.type === 'rest') {
        ...
        if (modal) {
            ...
            get('shop-close-btn').onclick = () => {
                ...
                NotificationManager.resumeGameFromModal('shop');
                // Also emit server-side shop close event
                socket.emit('closeShop');
            };
        ...
        // Fire a simple local event prompt using existing event modal pipeline
        const eventData = { 
            id: `evt_${Date.now()}`,
            type: 'npc',
            name: 'Mysterious Stranger',
            description: 'A hooded figure offers a risky bargain...',
            choices: [
                { label: 'Accept', description: 'Gain +1 AP next turn, risk 1 damage' },
                { label: 'Decline', description: 'Avoid the risk' }
            ]
        };
        if (typeof eventManager !== 'undefined') eventManager.showEvent(eventData);
    }
}
...
socket.on('turnStarted', ({ playerId }) => {
    ...
    // Defer YOUR TURN popup until modals/toasts are clear
    const waitUntilClear = () => {
        if (NotificationManager.isGated()) {
            return setTimeout(waitUntilClear, 200);
        }
        showYourTurnPopup();
        clientState.hasSeenSkillChallengePrompt = false;
        const myPlayer = currentRoomState.players[myId];
...
function showSpectatorRollAnimation(data) {
    ...
    const sides = parseInt((data.dice || 'd20').substring(1));
    const dieSVG = createDieSVG(sides, '?');
    dieSVG.classList.add('rolling');
...
socket.on('attackResolved', (data) => {
    ...
    if (data.rollerId === myId) {
        ...
        if (data.outcome === 'Miss') {
            ...
            clientState.rollModalCloseTimeout = setTimeout(() => modal.classList.add('hidden'), 2000);
        }
    } else {
...
socket.on('damageResolved', (data) => {
    ...
    if (data.rollerId === myId) {
        ...
        get('dice-roll-confirm-btn').classList.add('hidden');
        get('dice-roll-close-btn').classList.remove('hidden');
        clientState.rollModalCloseTimeout = setTimeout(() => modal.classList.add('hidden'), 4000);
    } else {
        updateSpectatorRollResult(data);
    }
});
...
socket.on('skillCheckResolved', (data) => {
    ...
    if (!modal.classList.contains('hidden')) {
        ...
        get('dice-roll-confirm-btn')?.classList.add('hidden');
        get('dice-roll-close-btn')?.classList.remove('hidden');
        clientState.rollModalCloseTimeout = setTimeout(() => modal.classList.add('hidden'), 2500);
    }
...
const combatGrid = {
            ...
            for (let x = 0; x < this.gridSize; x++) {
                ...
                entitiesHere.forEach(entity => {
                    ...
                    if (entity.type === 'player') {
                        // Always use sprite (no emoji fallback)
                        const spritePath = this.getSpritePath('player', entity.data.class);
                        
                        entityDiv.innerHTML = `
                            <img src="${spritePath}" class="grid-sprite pixel-sprite" alt="${entity.data.class}" loading="lazy">
                            <div class="grid-name">${entity.data.name}</div>
                        ...
                        // Always use sprite (no emoji fallback)
                        const spritePath = this.getSpritePath('monster', entity.data.name);
                        
                        entityDiv.innerHTML = `
                            <img src="${spritePath}" class="grid-sprite pixel-sprite monster-sprite" alt="${entity.data.name}" loading="lazy">
                            <div class="grid-name">${entity.data.name}</div>
                        ...
                        const spritePath = this.getSpritePath('companion', entity.data.name || 'companion');
                        entityDiv.innerHTML = `
                            <img src="${spritePath}" class="grid-sprite pixel-sprite" alt="${entity.data.name || 'Companion'}" loading="lazy">
                            <div class="grid-name">${entity.data.name || 'Companion'}</div>
                ...
                if (myPlayer && this.isValidMove(myPlayer, x, y)) {
                    ...
                    const moveHandler = () => this.handleMoveClick(x, y);
    ...
    getEmojiIcon(type, name) {
                ...
                if (type === 'player') {
                    ...
                    const monsterName = name.toLowerCase();
    ...
    handleMoveClick(x, y) {
        ...
        // Highlight the destination cell temporarily
    ...
    for (let y = 0; y < 5; y++) {
        for (let x = 0; x < 5; x++) {
            ...
            entitiesHere.forEach(entity => {
                ...
                if (entity.type === 'player') {
                    const spritePath = combatGrid.getSpritePath('player', entity.data.class);
                    entityDiv.innerHTML = `
                        <img src="${spritePath}" class="grid-sprite pixel-sprite player-sprite" alt="${entity.data.name}" loading="lazy">
                        <div class="grid-name">${entity.data.name}</div>
                        <div class="grid-hp player-hp">${entity.data.stats.currentHp}/${entity.data.stats.maxHp}</div>
                    `;
                    cell.classList.add('occupied-player');
                } else if (entity.type === 'monster') {
                    const spritePath = combatGrid.getSpritePath('monster', entity.data.name);
                    entityDiv.innerHTML = `
                        <img src="${spritePath}" class="grid-sprite pixel-sprite monster-sprite" alt="${entity.data.name}" loading="lazy">
                        <div class="grid-name">${entity.data.name}</div>
                        <div class="grid-hp monster-hp">${entity.data.currentHp}/${entity.data.maxHp}</div>
            ...
            if (myPlayer && combatGrid.isValidMove(myPlayer, x, y)) {
                ...
                const moveHandler = () => {
                    combatGrid.handleMoveClick(x, y);
...
combatGrid.render = function() {
    ...
    const mobileGridBtn = get('mobile-grid-btn');
    const hasMonsters = currentRoomState.gameState.board?.monsters?.length > 0;
