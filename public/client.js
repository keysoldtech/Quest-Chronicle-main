...
function switchMobileScreen(screenName) {
    ...
    try {
        if (document.querySelector('.mobile-tab-layout')) {
            localStorage.setItem('qc_mobile_tab', screenName);
        }
    } catch (_) { /* ignore */ }
}

/** Restore last mobile tab (experimental layout only). */
function initMobileTabFromStorage() {
    try {
        if (!document.querySelector('.mobile-tab-layout')) return;
        const saved = localStorage.getItem('qc_mobile_tab');
        if (saved && ['game', 'character', 'party', 'journal'].includes(saved)) {
            switchMobileScreen(saved);
        }
    } catch (_) { /* ignore */ }
}
