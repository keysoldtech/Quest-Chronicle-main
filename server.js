...
class GameManager {
    ...
    parseDiceString(diceString) {
        ...
        for (const part of parts) {
            ...
            if (trimmed.includes('d')) {
                ...
                const count = parseInt(countStr, 10) || 1; // Default to 1 die if not specified (e.g., "d6")
    ...
    resolveEventChoice(room, player, eventId, choiceIndex) {
        // Simplified event resolution
        const outcomes = [
            { type: 'damage', text: 'took some damage', value: -5 },
            { type: 'heal', text: 'found healing', value: 5 },
            { type: 'gold', text: 'found gold', value: 20 },
            { type: 'nothing', text: 'nothing happened', value: 0 },
            // Phase 1: risk/reward corruption boons
            { type: 'corruption_boon', text: 'accepted a dark boon', value: 1 },
        ];
        
        const outcome = outcomes[Math.floor(Math.random() * outcomes.length)];
        
        if (outcome.type === 'damage') {
            this._applyDamage(room, player, Math.abs(outcome.value), { name: 'Event' });
        } else if (outcome.type === 'heal') {
            player.stats.currentHp = Math.min(player.stats.maxHp, player.stats.currentHp + outcome.value);
        } else if (outcome.type === 'corruption_boon') {
            player.corruption = (player.corruption || 0) + outcome.value;
            // Grant immediate power (e.g., +2 all stats this run)
            Object.keys(player.inRunStatBonuses).forEach(k => player.inRunStatBonuses[k] += 2);
            // Apply a small persistent penalty while corrupted
            player.stats = this.calculatePlayerStats(player, room.gameState.partyHope);
        }
        
        room.chatLog.push({ 
            type: outcome.type === 'damage' ? 'combat' : 'action-good', 
            playerName: player.name, 
            text: `${player.name} made a choice and ${outcome.text}!`, 
            timestamp: Date.now() 
        });
        
        // CRITICAL FIX: Advance turn when event choice completes
        this.emitGameState(room.id);
        this.moveToNextTurn(room);
    }
